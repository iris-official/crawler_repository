...
import sys
AK = '0HR0GIfqMKczAWjpcI9BwoK1GBHVCX2q'
RANGE_LIMIT = '1000'
...
class SerchSchool(object):
    ...
    def __init__(self, CITY_CODE , CITY_NAME,PROVINCE_NAME, connection):
        ...
        self.BASE_DIR = 'result\\' + '学校' + str(CITY_NAME)
    ...
    def get_request(self,url, parameters, headers, num_retries=MAXTRY):
        ...
        try:
            ...
            if( html.text.find('"status":302')>-1 ):
                print('shool 天配额超限，限制访问"')
                easygui.msgbox("获取周边信息第一个程序学校超额！")
                sys.exit()
                    ...
                    for item in htm:
                        try:
                            ...
                            if(re.findall(pattern, item)):
                                ...
                                # & (name == key_word['name'])
                                if ((int(distance) != -1) & (int(distance) < int(RANGE_LIMIT))& (di_tag.find('教育') >-1)& (name == key_word['name'])):
                                    lat = coor[0].split(",")[0]
                                    lon = coor[0].split(",")[1]
                                    data["name"] = name
    ...
    def fun1(self,coor,id,find_list):
        ...
        if(banklist1 == []):
            mysql_helper.school1_info_sql_insert(self.CITY_NAME,self.PROVINCE_NAME,str(id) , str(poi[0])+','+str(poi[1]) , '' , '' , ''
                     , '', '', self.connection)
        else:
            for info in banklist1:
                mysql_helper.school1_info_sql_insert(self.CITY_NAME,self.PROVINCE_NAME,str(id) , str(poi[0])+','+str(poi[1]) ,str(info['name']),
                         str(info['coor'][0]) + ',' + str(info['coor'][1]) , str(info['dis']) ,
                          str(info['addr']) , str(info['di_tag']), self.connection)
        print(banklist1)
    ...
    def fun2(self,coor,id):
        ...
        if (banklist2 == []):
            mysql_helper.school2_info_sql_insert(self.CITY_NAME,self.PROVINCE_NAME,str(id) , str(poi[0]) + ',' + str(poi[1]) , '' , '' , ''
                     ,  self.connection)
        else:
            for info in banklist2:
                mysql_helper.school2_info_sql_insert(self.CITY_NAME,self.PROVINCE_NAME,str(id) , str(poi[0])+','+str(poi[1]) , str(info['name']) , str(info['lng']) + ',' + str(info['lat']) , str(info['addr']) , self.connection)
        print(banklist2)
        return banklist2
