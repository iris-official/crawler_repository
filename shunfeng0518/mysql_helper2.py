...
def select_danhao(connection):
    select_sql1 = 'select yjhm from generate_mail_no_2;'
    ...
    try:
        with connection.cursor() as cursor:
            ...
            result1_list = [item['yjhm'] for item in cursor.fetchall()] #���ɵ����е���
            cursor.execute(select_sql2)
            result2_list = [item['yjhm'] for item in cursor.fetchall()] #��ץ�����޼�¼����
            cursor.execute(select_sql3)
            result3_list = [item['yjhm'] for item in cursor.fetchall()] #��ץ�����м�¼����
            cursor.execute(select_sql4)
            result4_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����
            cursor.execute(select_sql5)
            result5_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����
            cursor.execute(select_sql6)
            result6_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����
            cursor.execute(select_sql7)
            result7_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����
            cursor.execute(select_sql8)
            result8_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����
            cursor.execute(select_sql9)
            result9_list = [item['yjhm'] for item in cursor.fetchall()]  # ��ץ�����м�¼����

            result_tmp1 = list(set(result1_list).difference(set(result2_list)))
            result_tmp2 = list(set(result_tmp1).difference(set(result3_list))) #�����ĵ���
