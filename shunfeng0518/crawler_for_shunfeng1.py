...
import numpy
import python.shunfeng0518.mysql_helper2 as mysql_helper2
...
if __name__ == '__main__':
    connection = mysql_helper2.mysql_login()
    danhao_list = mysql_helper2.select_danhao(connection)
    driver = browser_sets()
                ...
                if page == 'error': # 出现了验证码图片出不来、有alert框等情况
                    ...
                    if len(no_record_list) == GROUP_NUM:
                        ...
                        if empty_try < 0:
                            #driver = change_ip(driver)
                            empty_try = EMPTY_TRY
                            mysql_helper2.no_record_mail_insert2(no_record_list, connection)
                            index += GROUP_NUM
                        time.sleep(5)
                        continue
                    # 出错或无任何信息，重新爬
                    ...
                    if len(no_record_list) > 0:
                        mysql_helper2.no_record_mail_insert(no_record_list, connection)
                    if len(danhao_info_list) > 0:
                        mysql_helper2.mail_info_insert(danhao_info_list, connection)
                    if len(wuliu_info_list) > 0:
                        mysql_helper2.mail_wuliu_insert(wuliu_info_list, connection)
                    if len(terminal_list) > 0:
                        mysql_helper2.terminal_insert(terminal_list, connection)
                    print(index+1, 'OK')
                    max_try = MAX_TRY
                    empty_try = EMPTY_TRY
                    time.sleep(1)
    ...
    driver.quit()
    mysql_helper2.connection_close(connection)