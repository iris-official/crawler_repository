...
gzipped = result.info().get('Content-Encoding')
if gzipped:
    html_content = zlib.decompress(html_content, 16+zlib.MAX_WBITS)
pattern = re.compile(r'value=\"(LT-.*)\"')
lt = pattern.findall(html_content.decode('utf-8'))[0]
pattern = re.compile(r'name="execution" value="(.*)"')
execution = pattern.findall(html_content.decode('utf-8'))[0]
 

# data
data = {
    'username': '18810333522', #替换为自己账户的用户名
    'password': 'tian1314', #替换为自己账户的密码
    'execution': execution,
    '_eventId': 'submit',
    'lt': lt,
    'verifyCode': '',
...
req = urllib2.Request(auth_url, urllib.parse.quote(post_data).encode(encoding='UTF8'), headers)


try:
    # req = urllib.parse.urlencode(req).encode(encoding='UTF8')
    result = opener.open(req)
